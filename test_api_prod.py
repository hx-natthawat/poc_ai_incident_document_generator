"""Test script for the Incident Report Generator API - Production Environment."""
import json
import os
import requests
...
def test_api():
    ...
    base_url = "https://localhost"  # Production HTTPS
    headers = {
    ...
    print("Testing Incident Report Generator API (Production)...")
    
    # Test 1: Get sample data
    print("\n1. Testing GET /sample-data")
    try:
        response = requests.get(f"{base_url}/sample-data", headers=headers, verify=False)
    ...
    try:
        response = requests.get(f"{base_url}/reports/latest", headers=headers, verify=False)
